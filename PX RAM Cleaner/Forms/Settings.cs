...
using static PX_RAM_Cleaner.SaveSystem;
...
namespace PX_RAM_Cleaner
{
    public partial class Settings : FormWithShadow
    {
        ...
        void InitializeForm()
        {
            ...
            {
                if (MessageBox.Show(Translations.GetString("ResetSettings") + "?", "PX RAM Cleaner", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
        ...
        void InitializeTabs()
        {
            ...
            foreach (var item in tabsDict)
            {
                ...
                selector.MouseEnter += (s, e) => selector.BackColor = Color.FromArgb(0, 0, 0);
                selector.MouseLeave += (s, e) => selector.BackColor = tab.Visible ? Color.FromArgb(0, 0, 0) : Color.FromArgb(0, 0, 0);
                ...
                selector.Paint += (s, e) =>
                {
                    ...
                    using (var pen = new Pen(Color.FromArgb(0, 0, 0), 5))
            ...
            void SwitchTab(Panel selector, Panel tab)
            {
                ...
                foreach (var item in tabsDict)
                {
                    ...
                    {
                        item.Key.BackColor = Color.FromArgb(0, 0, 0);
                        item.Value.Visible = false;
                    }
                    else
                    {
                        selector.BackColor = Color.FromArgb(0, 0, 0);
                        tab.Visible = true;
                    }
                }
            ...
            AutorunCheck.Click += (s, e) =>
            {
                using (var key = Registry.CurrentUser.CreateSubKey(@"Software\Microsoft\Windows\CurrentVersion\Run"))
                {
                    if (AutorunCheck.Checked)
                        key.SetValue("PX RAM Cleaner", $"\"{Paths.ApplicationExe}\" silent");
                    else
                        key.DeleteValue("PX RAM Cleaner", false);
                }
            };
        ...
        void CheckSettings()
        {
            ...
            {
                if (key?.GetValue("PX RAM Cleaner") != null)
        ...
        private void AboutLink3_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
